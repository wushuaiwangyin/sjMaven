...
function resetTreeCRUDPageHeight() {
	var h = getCenterHeight() - 43;

	if (getCenterHeight() - 43 < $("#list-page").height()) {
		h = $("#list-page").height();
	}
...
function operatorBtn2(value, row) {
	return $("#rowBtn_edit").html()
		...
		if (mode == 'del' || mode == 'delete' || mode == 'delSelect') {
			...
			if (mode == 'delSelect') {
				var rows = $('#' + params.table).bootstrapTable(
						"getAllSelections");
				if (rows.length <= 0) {
					layer.msg('<span class="red bigger-120">请选择需要删除的数据</span>');
					return;
				} else {
					var ids = [];
												...
												function(data) {
													...
													if (data > 0) {
														if (params.success == undefined) {
															layer
																	.msg(
																			'<span class="red bigger-120">删除成功</span>',
																			1,
																			1,
																			function() {
																				if (params.reloadurl) {
														...
														layer
																.alert(
																		'<span class="red bigger-120">删除失败，数据正在被使用！</span>',
																		8, !1);
													}
												})
										.fail(
												function(error) {
													layer
															.msg(
																	'<span class="red bigger-120">删除失败</span>',
																	2, 8);
												});
							}, params.title);
			return false;
			...
			$
					...
					.done(
							function(data) {
								layer.close(loadi);
								if (data > 0) {
									layer.msg('核实成功', 1, 1,function(){
										layer.closeAll();
	                            	});
								}
								else if (data == 0) {
									layer.msg(
											'<span class="red bigger-120">数据已核实，请勿重复核实</span>',3, 1);
								}else if (data == -1) {
									layer.msg(
											'<span class="red bigger-120">核实失败</span>',3, 1);
								}else if (data == -2) {
									layer.msg(
											'<span class="red bigger-120">数据已下发,请耐心等待下级机构处理</span>',3, 1);
								}
							})
							.fail(
									);
...
//===============================================================================================================================
		...
		if (mode == 'send') {
			...
			if (rows.length <= 0) {
				...
				for (var i = 0; i < rows.length; i++) {
					...
					idstr = (idstr + rows[i].dataid) + (((i + 1)== rows.length) ? '':','); 
					organId = rows[i].organid;
		...
		if (mode == 'add') {
			...
			if(codePre == 0){
				layer.msg('<span class="red bigger-120">请先点击左侧类型再进行创建</span>');
				return;
			}
			params.data = {
				codePre : codePre,
		...
		//========================================================================================================
		if (mode == 'tableDetails') {
			var organId = $("#organId").val();
			var custNo = $("#custNo").val();
//			var dealStatus = $("#status-chosen").val();
...
//					dealStatus : dealStatus,
		...
		if (mode == 'findDetails') {
			...
			if (rows.length <= 0) {
				...
				layer.msg('<span class="red bigger-120">请选择一条数据查看详细信息</span>');
				return;
			} else {
				var idstr = "";
				var modelId;
				for (var i = 0; i < rows.length; i++) {
					idstr = (idstr + rows[i].数据编号) + (((i + 1)== rows.length) ? '':','); 
					modelId = rows[i].模型编号;
				}
				params.data = {
					modelId : modelId,
...
function tableLoadSuccess(data){
	var ps = $('#table').bootstrapTable.defaults.pageSize * 46.6;
...
function downloadFile(formid, action) {
	var table = $("#" + formid);
