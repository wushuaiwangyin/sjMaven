	...
	if($.Datatype){
		...
		$.extend($.Datatype,{
			...
			*/
			...
			"combotree-must":function(gets,obj,curform,datatype){
				var values = curform.find("#"+obj.attr("id")).combotree("getValues");
				if(values.length <= 0){
					return false;
			...
			"numMin":function(gets,obj,curform,datatype){
				...
				var minV = 0;
				if(obj.attr("check").length > 0){
					minV = $.trim(curform.find("*[name='"+obj.attr("check")+"']").val());
					if(minV.length <= 0){
						return true;
					}
				}
			...
			"numMax":function(gets,obj,curform,datatype){
				...
				var max = 10000000000;
				if(obj.attr("check").length > 0){
					max = $.trim(curform.find("*[name='"+obj.attr("check")+"']").val());
					if(max.length <= 0){
						return true;
					}
				}
