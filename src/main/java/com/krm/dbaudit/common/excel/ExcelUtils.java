package com.krm.dbaudit.common.excel;
...
import com.krm.dbaudit.common.excel.template.PoiTemplate;
...
public class ExcelUtils {
	 ...
	 * @param titleMap (key:"列名",value = "属性名")
	 */
	public static void exportMyExcel(HttpServletResponse response,
			String fileName, List<?> data, Map titleMap) throws Exception {
		ByteArrayOutputStream os = new ByteArrayOutputStream();
		PoiUtil.writeMyExcel(data, os, titleMap);
		os.flush();
		byte[] buf = os.toByteArray();
		InputStream is = new ByteArrayInputStream(buf, 0, buf.length);
		FileUtils.downloadFile(response, is, fileName);
