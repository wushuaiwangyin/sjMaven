...
public class SysRoleService extends ServiceMybatis<SysRole> {
	...
	public int saveSysRole(SysRole sysRole){
		...
		if(null == sysRole.getId()){
			Long id = sysRoleMapper.generateId();
			sysRole.setId(id);
			sysRole.set("createDate",new Date() );
			sysRole.set("createBy", SysUserUtils.getCacheLoginUser().getId()+","+
					SysUserUtils.getCacheLoginUser().getName());
			sysRole.set("delFlag", Constant.DEL_FLAG_NORMAL);
